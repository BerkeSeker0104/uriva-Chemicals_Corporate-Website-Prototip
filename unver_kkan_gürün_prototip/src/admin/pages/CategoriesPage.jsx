...
const emptyCategory = { name: { tr: '', en: '' }, sortOrder: 0, active: true }
...
export default function CategoriesPage() {
  ...
  const reload = () => setItems(db.categories.getAll())
  useEffect(reload, [])

  const openNew = () => { setForm({ ...emptyCategory, name: { tr: '', en: '' } }); setEditing(null); setModalOpen(true) }
  const openEdit = (row) => { setForm({ ...row }); setEditing(row.id); setModalOpen(true) }

  const handleSubmit = () => {
    if (editing) db.categories.update(editing, form)
    else db.categories.create(form)
    setModalOpen(false)
    reload()
  }
  ...
  const columns = [
    { header: 'Kategori Adı (TR)', accessor: r => r.name?.tr },
    { header: 'Kategori Adı (EN)', accessor: r => r.name?.en },
  ...
  return (
    <div>
      ...
      <FormModal title={editing ? 'Kategori Düzenle' : 'Yeni Kategori'} isOpen={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleSubmit}>
        <LangTabs>
          {(lang) => (
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Kategori Adı ({lang.toUpperCase()})</label>
              <input
                type="text"
      ...
      <ConfirmDialog isOpen={!!deleteTarget} message={`"${deleteTarget?.name?.tr}" kategorisini silmek istediğinize emin misiniz?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
    </div>
  )
}
